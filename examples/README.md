...
| [Enterprise & Oracle DDL](#enterprise-oracle-and-dynamodb-ddl) | 11 | PostgreSQL ledger, multi-file Oracle packs, standalone Amazon DynamoDB CloudFormation |
...
Upload **`.sql` / `.ddl` / `.yaml`** for paste import, **`.db`** for SQLite introspection, or pick any row below in **Load example**.

All **Amazon DynamoDB** entries in the picker use the label prefix `Amazon DynamoDB (CloudFormation) - …` (see [Amazon DynamoDB CloudFormation examples](#amazon-dynamodb-cloudformation-examples)).
...
| Amazon DynamoDB orders | [`dynamodb/orders-table.yaml`](dynamodb/orders-table.yaml) | `dynamodb` | Composite-key orders table with TTL and GSI date index |
...
### Amazon DynamoDB CloudFormation examples

Four **Load example** entries import Amazon DynamoDB single-table (or focused) CloudFormation templates. Picker labels always start with **Amazon DynamoDB (CloudFormation)**.

| Load example label | File | Suggested profile | Single-table resource | GSIs | Notes |
| --- | --- | --- | --- | ---: | --- |
| Amazon DynamoDB (CloudFormation) - IoT | [`dialects/dynamodb.yaml`](dialects/dynamodb.yaml) | `iot` | `IoTPlatformTable` | 3 | Fleet status, global alerts, firmware audit; TTL `ExpireAt`; streams + PITR + SSE. Per-dialect **bucket** pattern demo. |
| Amazon DynamoDB (CloudFormation) - CMS Platform | [`dynamodb/cms-platform-table.yaml`](dynamodb/cms-platform-table.yaml) | `cms` | `CmsPlatformTable` | 3 | Publish feed/taxonomy, author/moderation queue, slug/asset routing. GSI2 `INCLUDE` projection — see [GSI → MongoDB migration](../docs/20-dynamodb-gsi-mongodb-migration.md). |
| Amazon DynamoDB (CloudFormation) - Ecommerce Catalog | [`dynamodb/ecommerce-catalog-table.yaml`](dynamodb/ecommerce-catalog-table.yaml) | `catalog` | `EcommerceCatalogTable` | 3 | Category/price, SKU/brand, seller/status access patterns; TTL on catalog snapshots. |
| Amazon DynamoDB (CloudFormation) - Orders | [`dynamodb/orders-table.yaml`](dynamodb/orders-table.yaml) | — | `Production-Orders` | 1 | Classic multi-attribute orders table (`CustomerId` + `OrderId`); `DateIndex` GSI. |

```bash
# Migration Studio: Load example → Amazon DynamoDB (CloudFormation) - CMS Platform
# CLI paste import:
npm run hvymetl -- design --ddl-file examples/dynamodb/cms-platform-table.yaml --profile cms --out out/cms-dynamo
```

**GSI → MongoDB index mapping:** [`docs/20-dynamodb-gsi-mongodb-migration.md`](../docs/20-dynamodb-gsi-mongodb-migration.md) documents compound indexes (covered queries) and Atlas Search `storedSource` equivalents, with unit tests in `src/utilities/dynamoGsiMongoMigration.test.ts`.

**Regenerate** the per-dialect IoT bundle after template edits: `npm run generate-dialect-examples` (validates with `src/examples/dialectExamples.test.ts`).
...
| [`bucket.md`](../knowledge/bucket.md) | `bucket` | `iot` | `iot` | 60k-row `sensor_readings` → native time series collection | `design --source examples/iot/iot.db --profile iot` |
| [`bucket.md`](../knowledge/bucket.md) | `bucket` | `dialects/dynamodb.yaml` | `iot` | Single-table IoT telemetry platform (`IoTPlatformTable`, TTL, 3 GSIs) | Load example **Amazon DynamoDB (CloudFormation) - IoT** |
| [`bucket.md`](../knowledge/bucket.md) | `bucket` | `dialects/json-schema.json` | `iot` | JSON Schema bundle mirroring IoT bucket shape | Load example **JSON Schema - IoT** |
...
| [`polymorphic.md`](../knowledge/polymorphic.md) | `polymorphic` | `dialects/redshift.sql` | `cms` | CMS polymorphic DDL shape (per-dialect demo) | Load example **Amazon Redshift - CMS** |
...
| [`polymorphic.md`](../knowledge/polymorphic.md) | `polymorphic` | `dynamodb/cms-platform-table.yaml` | `cms` | Single-table CMS (`CmsPlatformTable`); editorial + moderation GSIs | Load example **Amazon DynamoDB (CloudFormation) - CMS Platform** |
...
| `dynamodb/cms-platform-table.yaml` | `cms` | single-table CMS; semantic GSI keys (`gSI2AuthorModerationIndex`, …) — [GSI migration doc](../docs/20-dynamodb-gsi-mongodb-migration.md) |
| `dynamodb/ecommerce-catalog-table.yaml` | `catalog` | single-table catalog; category/SKU/seller GSIs |
| `dialects/dynamodb.yaml` | `iot` | single-table IoT platform; bucket pattern; TTL + streams |
