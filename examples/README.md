...
domain ships DDL (`.sql`), a deterministic seeder ([`src/examples/seed.ts`](../src/examples/seed.ts)),
...
| [`computed.md`](../knowledge/computed.md) | `computed` | *(all seven)* | each domain’s default profile | Counter fields on parents (e.g. `totalReviews`, `count` on buckets) | any `design` run above |
| [`embed-vs-reference.md`](../knowledge/embed-vs-reference.md) | `embed` / `reference` | *(all seven)* | each default profile | Bounded children embedded; unbounded or lookup paths referenced | inspect `embeddedArrays` / FK decisions in the plan |
...
| [`schema-versioning.md`](../knowledge/schema-versioning.md) | `schema-versioning` | *(all seven)* | each default profile | Stamped on every planned collection | any `design` run |
...
Run `npm test -- src/examples/examplePatternCoverage.test.ts` to regression-check
...
### Oracle DDL (diagram only)

[`oracle/`](oracle/) provides Oracle DDL and a Migration Studio diagram for
dialect demos. It is **not** part of `seed-examples` and has no bundled `.db` file.
