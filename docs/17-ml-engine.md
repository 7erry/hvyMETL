...
#### Lesson `text` field: format, provenance, and accuracy

Documents in `hvymetl_memory.hvymetl_lessons_learned` often look like:

```text
PERFORMANCE WARNING: Table 'posts' migrated using Attribute + Reference resulted in 18.2% cache miss rate and 142 slow queries under workload Read-Heavy (95:5) | High Throughput (60,000 RPM) | Growth: … Predicted cache-miss 6.0% vs actual 18.2%. Root causes: actualCacheMissRate 18.2% exceeds 15%; … Recommendation: avoid Attribute + Reference for read-heavy telemetry at 60000 RPM; prefer reference, bucket, or subset patterns instead.
```

That string is **generated by hvyMETL**, not copied from Atlas or written manually.

**Write path**

1. **Log decision** — ML design (`designFromModelWithMlEngine` / `runMlEnhancedDesign` in [`pipelinePatch.ts`](../src/ml_engine/pipelinePatch.ts)) calls [`logMigrationPlanDecisions()`](../src/ml_engine/feedbackCollector.ts). One row per collection lands in `hvymetl_migration_logs` with `patternsApplied`, `predictedMetrics`, and workload telemetry.
2. **Reflect** — When reflection is scheduled (`HVYMETL_SCHEDULE_REFLECTION=1`, pipeline auto-schedule, or `hvymetl reflect --migration-id`), [`analyzeAndReflect()`](../src/ml_engine/feedbackCollector.ts) fetches post-migration metrics and compares them to [reflection thresholds](#reflection-thresholds).
3. **Upsert lesson** — On breach, [`formatLessonLearnedText()`](../src/ml_engine/memoryEngine.ts) builds the prose and [`upsertLessonLearned()`](../src/ml_engine/memoryEngine.ts) persists it (optionally with an embedding for retrieval).

**How each phrase is derived**

| Fragment in `text` | Source |
| --- | --- |
| `Table 'posts'` | MongoDB collection name from the logged decision (`collectionName` / plan name) |
| `Attribute + Reference` | `patternsApplied` from the migration plan — pattern ids title-cased and joined (`attribute` + `reference` → `Attribute + Reference`) |
| `PERFORMANCE WARNING` vs `CRITICAL FAILURE` | [`classifySeverity()`](../src/ml_engine/memoryEngine.ts): `critical` when cache miss > 20%, slow queries > 200, or IOPS breach; otherwise `warning` if any threshold fired |
| Cache miss %, slow query count, IOPS | `actualMetrics` from the metrics connector (see below) |
| `Root causes: …` | [`analyzeMetrics()`](../src/ml_engine/feedbackCollector.ts) breach list |
| `Recommendation: avoid …` | Fixed template in `formatLessonLearnedText()` — heuristic guidance for future designs |

**What is accurate vs heuristic**

| Claim in `text` | Trust level |
| --- | --- |
| Collection/table name | **Reliable** — mirrors the logged migration plan |
| Patterns used (`Attribute + Reference`, etc.) | **Reliable** — mirrors `patternsApplied` / `patternsInvolved` on the lesson document |
| That a reflection threshold was breached | **Reliable** — lessons are only written when `analyzeMetrics()` sets `breached: true` |
| Numeric cache miss / slow-query figures | **Depends on metrics source** (see below) |
| “Resulted in … because of this pattern” | **Correlation, not causation** — attributes cluster-level or stubbed signals to the design choice; use for RAG steering, not as an Atlas RCA report |

**Metrics sources (`actualMetrics.source`)**

| Connector | When | Accuracy notes |
| --- | --- | --- |
| **`StubAtlasMetricsConnector`** | Default when live Atlas Admin API is not registered; also forced by `HVYMETL_ATLAS_STUB_MODE=healthy\|degraded` | **Synthetic.** Derives “actual” values from critic predictions and workload heuristics (write-heavy + embed-like patterns get a stress multiplier). `degraded` always breaches. **Not real cluster telemetry.** |
| **`AtlasApiMetricsConnector`** | `ATLAS_CLIENT_ID`, `ATLAS_CLIENT_SECRET`, `ATLAS_GROUP_ID`, and `HVYMETL_ATLAS_CLUSTER_ID` set; stub mode not forced ([`atlasApiMetrics.ts`](../src/ml_engine/atlasApiMetrics.ts)) | **Live Atlas Admin API.** Slow-query count can filter to `targetDatabase.targetCollection` when `atlasCorrelation` is on the migration log. Cache miss and IOPS are **process/cluster-level**, not isolated to one collection. |

**Auditing a specific lesson**

1. Read the sibling document in `hvymetl_migration_logs` (match `migrationId` or `lessonLearnedId` on the log).
2. Inspect `patternsApplied`, `predictedMetrics`, `actualMetrics`, and `actualMetrics.source`.
3. Read `reflectionNotes` on the log for the exact breach strings copied into `Root causes`.

Retrieved lessons are injected into future design prompts as **HISTORICAL LESSONS LEARNED FROM PAST MIGRATIONS** via [`retrieveLessonsLearned()`](../src/ml_engine/memoryEngine.ts) and [`renderHistoricalLessonsSection()`](../src/ml_engine/memoryEngine.ts). They steer the RAG/design engine away from repeated pattern choices under similar telemetry; they are not a substitute for Atlas Performance Advisor or query profiling.
