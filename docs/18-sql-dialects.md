...
hvyMETL supports **twenty-four schema import dialects** (twenty-two SQL DDL dialects plus
**Amazon DynamoDB CloudFormation** and **JSON Schema**). They fall into two capability tiers:
...
2. **DDL / template paste** — **all twenty-four dialects**: paste `CREATE TABLE` scripts,
   DynamoDB CloudFormation templates, or JSON Schema bundles; parsers build the same
   `SqlStructuralModel` the design engine consumes. Row counts default to zero; relationship
   stats use conservative defaults.
...
| `mariadb` | MariaDB | DDL paste | No | MySQL-compatible; `AUTO_INCREMENT`, inline FKs |
...
| `yugabyte` | YugabyteDB | DDL paste | No | PostgreSQL-compatible; `UUID`, inline `REFERENCES` |
...
| `redshift` | Amazon Redshift | DDL paste | No | PostgreSQL-compatible; `IDENTITY`, `BIGINT`, inline FKs |
| `snowflake` | Snowflake | DDL paste | No | `AUTOINCREMENT`, `VARIANT`, `NUMBER`, inline FKs |
| `bigquery` | Google BigQuery | DDL paste | No | Backtick-qualified names, trailing `PRIMARY KEY`, `INT64` / `STRING` |
| `spanner` | Google Cloud Spanner | DDL paste | No | Trailing `PRIMARY KEY (…)`, `INT64` / `STRING` / `BYTES` |
| `databricks` | Databricks SQL / Spark SQL | DDL paste | No | `USING DELTA`, table-level FK constraints |
| `dynamodb` | Amazon DynamoDB (CloudFormation) | Template paste (YAML/JSON) | No | `AWS::DynamoDB::Table` — keys, GSIs, TTL attributes. GSI → MongoDB mapping: [docs/20-dynamodb-gsi-mongodb-migration.md](./20-dynamodb-gsi-mongodb-migration.md) |
| `json-schema` | JSON Schema | JSON paste | No | Draft 2020-12 object schemas; `$ref` → FK edges ([doc](19-json-schema-import.md)) |
| `singlestore` | SingleStore (MemSQL) | DDL paste | No | MySQL-compatible; `AUTO_INCREMENT` |
| `sap-hana` | SAP HANA | DDL paste | No | `NVARCHAR`, `GENERATED BY DEFAULT AS IDENTITY`, table-level FKs |
| `teradata` | Teradata | DDL paste | No | `CREATE MULTISET TABLE`, table-level PK/FK |
| `firebird` | Firebird | DDL paste | No | Inline `REFERENCES`, integer PKs |

The dialect selector in Migration Studio sets the model `source` label (e.g.
`ddl:snowflake`) and documents which SQL syntax to paste; parsing is shared with
dialect-specific handling for qualified names, quoted FK targets, Spanner-style
primary keys, Teradata `MULTISET TABLE`, and warehouse-specific column types.
...
flowchart TB
    ...
    subgraph ddl [DDL / template paste — all 24 dialects]
        PASTE[Pasted CREATE TABLE, CFN template, or JSON Schema] --> PARSER[ddlParser / dynamodbCloudFormationParser / jsonSchemaParser]
...
### `DIALECTS`, validation, and `isLiveSourceDialect(dialectId)`

| Export | Description |
| --- | --- |
| `DIALECTS` | Array of `{ id, label, live, parserFamily }` — `live: true` only for SQLite |
| `SUPPORTED_DIALECT_IDS` | Canonical id list used by tests and API validation |
| `normalizeDialectId(id)` | Lowercases and resolves aliases (`spark-sql` → `databricks`, `memsql` → `singlestore`) |
| `isSupportedDialect(id)` | `true` when the id (or alias) is registered |
| `resolveImportDialect(id)` | Returns canonical id; throws on unknown ids; defaults to `postgresql` when omitted |
...
`resolveImportDialect` and returns `400` when the id is not supported.

### `parseSchemaImport(content, dialect, sourceLabel): SqlStructuralModel`

[`src/utilities/schemaImport.ts`](../src/utilities/schemaImport.ts) routes imports by dialect.
SQL dialects use [`parseDdlToModel`](../src/utilities/ddlParser.ts). DynamoDB uses
[`parseDynamoDbCloudFormationToModel`](../src/utilities/dynamodbCloudFormationParser.ts) for
YAML or JSON CloudFormation templates containing `AWS::DynamoDB::Table` resources. JSON Schema
...
1. Select dialect in the sidebar (e.g. **Snowflake**).
...
curl -X POST http://localhost:3847/api/schema/import-ddl \
  ...
  -d '{"dialect":"snowflake","ddl":"CREATE TABLE orders (id NUMBER PRIMARY KEY, …);"}'
```
...
### Dialect pattern examples (all 24 import dialects)

```text
examples/dialects/
  README.md               # dialect → pattern matrix
  {dialect}.sql           # one file per SQL dialect
  dynamodb.yaml           # CloudFormation for DynamoDB (see docs/20-dynamodb-gsi-mongodb-migration.md)
  json-schema.json        # JSON Schema bundle (draft 2020-12)
```
