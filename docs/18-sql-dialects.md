...
hvyMETL supports **twenty-three schema import dialects** (twenty-two SQL DDL dialects plus
**Amazon DynamoDB CloudFormation**). They fall into two capability tiers:
...
2. **DDL / template paste** — **all twenty-three dialects**: paste `CREATE TABLE` scripts or
   DynamoDB CloudFormation templates; parsers build the same `SqlStructuralModel` the design
   engine consumes. Row counts default to zero; relationship stats use conservative defaults.
...
flowchart TB
    ...
    subgraph ddl [DDL / template paste — all 23 dialects]
        PASTE[Pasted CREATE TABLE or CFN template] --> PARSER[ddlParser / dynamodbCloudFormationParser]
...
YAML or JSON CloudFormation templates containing `AWS::DynamoDB::Table` resources.
