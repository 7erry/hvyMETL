...
/** Human-readable warnings when CSV basenames do not align with SQL table names. */
export function csvTableMatchWarnings(csvFileNames: string[], expectedTableNames: string[]): string[] {
  ...
  if (missingTables.length > 0) {
    ...
    warnings.push(
      `Missing CSV exports for ${missingTables.length} table(s): ${listed}${missingTables.length > 6 ? '…' : ''}.`,
    );
  }
