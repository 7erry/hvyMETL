...
describe('explainTransformation', () => {
  it('warns on DDL-only stats and explains aggressive embed defaults for read-heavy profile', () => {
    ...
    expect(summary.foldedTableCount).toBeGreaterThan(0);
    expect(summary.insights.some((insight) => insight.title.includes('DDL-only'))).toBe(true);
    expect(summary.insights.some((insight) => insight.title.includes('Read-heavy profile'))).toBe(true);
    expect(summary.markdown).toContain('# Transformation Summary');
  });

  it('warns when profile is write-heavy enough to keep unbounded children as references', () => {
    const model = parseDdlToModel(BANKING_DDL, 'ddl:mysql');
    const plan = buildMigrationPlan(model, getProfile('iot'));
    const summary = explainTransformation(model, model, plan, getProfile('iot'));

    expect(summary.readHeavyEligible).toBe(false);
    expect(summary.writeHeavy).toBe(true);
    expect(summary.insights.some((insight) => insight.title.includes('Write-heavy profile'))).toBe(true);
  });
});
